...
def max(input, axis=None, keep_dims=False, propagate_nan: core.constexpr = core.PropagateNan.NONE):
...
@jit
def _max_with_indices(input, axis, tie_break_left=True, keep_dims=False):
    if tie_break_left:
...
def argmax(input, axis, tie_break_left=True, keep_dims=False):
    (_, ret) = _max_with_indices(input, axis, tie_break_left, keep_dims)
    return ret
...
def min(input, axis=None, keep_dims=False, propagate_nan: core.constexpr = core.PropagateNan.NONE):
    input = core._promote_bfloat16_to_float32(input)
...
@jit
def _min_with_indices(input, axis, tie_break_left=True, keep_dims=False):
    if tie_break_left:
...
def argmin(input, axis, tie_break_left=True, keep_dims=False):
    (_, ret) = _min_with_indices(input, axis, tie_break_left, keep_dims)
    return ret
